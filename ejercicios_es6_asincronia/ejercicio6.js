...
const sumaAprovados = exams.reduce((acc, exam) => {
    if (exam.score >= 5) {
        acc += exam.score;
    }
    return acc
}, 0)
console.log(sumaAprovados)


/*6.3 Dado el mismo array, haz la media de las notas de todos los examenes .reduce().*/

const sumaTotal = exams.reduce((acc, exam) => acc + exam.score, 0)
const notasMedia = sumaTotal / exams.length;
